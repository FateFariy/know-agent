...
const (
	tokenRatio      = 4   // token 估算系数（中文字符数/系数≈token数）
...
// 		return err
// 	}
// 	if err = d.repo.UpdateStepExecuteStatus(ctx, planId, vo.ExecuteStatusRunning); err != nil {
// 		Warnf("更新步骤执行状态失败: planId=%d, err=%v", planId, err)
// 	}
//
// 	// 执行切块流水线
...
// 	if err != nil {
// 		Warnf("执行切块流水线失败: documentId=%d, err=%v", documentId, err)
// 		d.handleIndexBuildFailure(ctx, document, task, plan, err.Error())
// 		return err
// 	}
//
...
// 	}
//
// 	finalCandidates := cleanupParentCandidates(candidates)
...
// 		tokenCount := charCount / tokenRatio
// 		for _, child := range candidate.ChildChunks {
// 			charCount += len(child.Text)
// 			tokenCount += len(child.Text) / tokenRatio
// 		}
//
// 		parentNo := parentIdx + 1
// 		parentBlocks = append(parentBlocks, &entity.DocumentParentBlock{
// 			ID:                parentBlockId,
// 			DocumentId:        documentId,
// 			TaskId:            taskId,
// 			PlanId:            planId,
...
// 			EndChunkNo:        globalChunkNo + childCount - 1,
// 		})
//
// 		for localIdx, child := range candidate.ChildChunks {
...
// 				ChunkNo:           globalChunkNo + localIdx,
...
// 				ItemIndex:         int64(child.ItemIndex),
...
func (d *AsyncProcessingLogicImpl) handleIndexBuildFailure(ctx context.Context, document *entity.Document, task *entity.DocumentTask, plan *entity.DocumentStrategyPlan, errorMsg string) {
	...
	fn := func(txCtx context.Context) error {
		...
		chunk := &entity.DocumentChunk{TaskId: task.ID, VectorStatus: vo.VectorStatusFailed, VectorStoreType: vo.VectorStoreTypeMilvus}
...
// func cleanupParentCandidates(candidates []*vo.ParentBlockCandidate) []*vo.ParentBlockCandidate {
// 	result := make([]*vo.ParentBlockCandidate, 0, len(candidates))
...
// 		hasValidChild := false
// 		for _, child := range candidate.ChildChunks {
// 			if child != nil && strings.TrimSpace(child.Text) != "" {
// 				hasValidChild = true
// 				break
// 			}
// 		}
