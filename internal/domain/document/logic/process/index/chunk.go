...
func (p *ChunkingPhase) cloneChunkCandidate(original *vo.ChunkCandidate, text string) *vo.ChunkCandidate {
	...
	candidate := *original
	candidate.Text = text
	candidate.Keywords = strings.TrimSpace(original.Keywords)
	var keywords []string
	if candidate.Keywords == "" {
		keywords = candidate.ExtractKeywords(p.tokenizer)
		candidate.Keywords = marshal(keywords)
	}
	candidate.Questions = strings.TrimSpace(original.Questions)
	if candidate.Questions == "" {
		if len(keywords) == 0 {
			keywords = parseJsonArray(candidate.Keywords)
		}
